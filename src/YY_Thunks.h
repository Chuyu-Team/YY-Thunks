...
__declspec(allocate(".YYThu$AAB")) static void *__YY_THUNKS_FUN_START[] = {
    nullptr}; //鸭船指针缓存开始位置
__declspec(allocate(".YYThu$AAC")) static void *__YY_THUNKS_FUN_END[] = {
    nullptr}; //鸭船指针缓存结束位置

__declspec(allocate(".YYThr$AAA")) static void *__YY_THUNKS_INIT_FUN_START[] = {
    nullptr}; //鸭船指针缓存结束位置
__declspec(allocate(".YYThr$AAB")) static void *__YY_THUNKS_INIT_FUN_END[] = {
    nullptr}; //鸭船指针缓存结束位置
...
template <int Flags>
static HMODULE __fastcall try_get_module(
...
static void *__fastcall try_get_function(
    ...
    try_get_module_fun get_module) noexcept
{
...
static void __cdecl __YY_uninitialize_winapi_thunks()
{
    ...
    if (auto pRtlDllShutdownInProgress = (decltype(RtlDllShutdownInProgress) *)GetProcAddress(
            try_get_module_ntdll(), "RtlDllShutdownInProgress"))
    {
...
static int __cdecl _YY_initialize_winapi_thunks()
{
     ...
     */
    for (auto p = (InitFunType *)__YY_THUNKS_INIT_FUN_START;
         p != (InitFunType *)__YY_THUNKS_INIT_FUN_END;
         ++p)
    {
        if (auto pInitFun = *p)
            pInitFun();
