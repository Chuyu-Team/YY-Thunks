...
int __cdecl wmain(int argc, wchar_t* argv[])
{
    ...
    if (_wcsnicmp(szMinimumRequiredVersion, L"/MinimumRequiredVersion:", _countof(L"/MinimumRequiredVersion:") -1) == 0)
    {
    ...
    if (swscanf(szMinimumRequiredVersion, L"%hu.%hu", MinimumRequiredVersion + 1, MinimumRequiredVersion + 0) != 2)
    {
    ...
    auto hFile = CreateFileW(szFilePath, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, 0, nullptr);
    ...
    do
    {
        ...
        if (Header32.FileHeader.Machine != IMAGE_FILE_MACHINE_I386
            || UFIELD_OFFSET(decltype(Header32.OptionalHeader), DataDirectory) > Header32.OptionalHeader.SizeOfHeaders)
        {
            wprintf(L"%ws 不是i386 PE文件，无需更新。\n", szFilePath);
            break;
        }

        if (Header32.OptionalHeader.Subsystem != IMAGE_SUBSYSTEM_WINDOWS_GUI && Header32.OptionalHeader.Subsystem != IMAGE_SUBSYSTEM_WINDOWS_CUI)
        {
        ...
        if (MakeMiniVersion(Header32.OptionalHeader.MajorOperatingSystemVersion, Header32.OptionalHeader.MinorOperatingSystemVersion) > *(DWORD*)MinimumRequiredVersion)
        {
        ...
        if (MakeMiniVersion(Header32.OptionalHeader.MajorSubsystemVersion, Header32.OptionalHeader.MinorSubsystemVersion) > *(DWORD*)MinimumRequiredVersion)
        {
        ...
        SetFilePointer(hFileWriter, DosHeader.e_lfanew + UFIELD_OFFSET(decltype(Header32), OptionalHeader), nullptr, FILE_BEGIN);

        auto& cbWriter = cbRead;
        if (WriteFile(hFileWriter, &Header32.OptionalHeader, UFIELD_OFFSET(decltype(Header32.OptionalHeader), DataDirectory), &cbWriter, nullptr))
        {
            wprintf(L"%ws 最低支持版本成功更新到 %hu.%hu！\n", szFilePath, MinimumRequiredVersion[1], MinimumRequiredVersion[0]);
        }
        else
        {
            lStatus = GetLastError();
