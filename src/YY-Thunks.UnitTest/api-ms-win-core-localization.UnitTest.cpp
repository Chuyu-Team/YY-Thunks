...
namespace api_ms_win_core_localization
{
    ...
    public:
        FindNLSStringEx()
        {
            Guard |= YY::Thunks::aways_null_try_get_FindNLSStringEx;
        }

        TEST_METHOD(FIND_STARTSWITH验证)
        {
            INT _nFound;
            int _nIndex;

            _nIndex = ::FindNLSStringEx(LOCALE_NAME_SYSTEM_DEFAULT, FIND_STARTSWITH, L"0123456789", -1, L"0123", -1, &_nFound, nullptr, nullptr, 0);
            Assert::AreEqual(_nIndex, 0);
            Assert::AreEqual(_nFound, 4);

            _nIndex = ::FindNLSStringEx(LOCALE_NAME_SYSTEM_DEFAULT, FIND_STARTSWITH, L"0123456789", -1, L"1234", -1, &_nFound, nullptr, nullptr, 0);
            Assert::AreEqual(_nIndex, -1);

            _nIndex = ::FindNLSStringEx(LOCALE_NAME_SYSTEM_DEFAULT, FIND_STARTSWITH, L"0123456789", -1, L"6789", -1, &_nFound, nullptr, nullptr, 0);
            Assert::AreEqual(_nIndex, -1);
        }

        TEST_METHOD(FIND_FROMSTART验证)
        {
            INT _nFound;
            int _nIndex;
        ...
        TEST_METHOD(FIND_ENDSWITH验证)
        {
            INT _nFound;
            int _nIndex;

            _nIndex = ::FindNLSStringEx(LOCALE_NAME_SYSTEM_DEFAULT, FIND_ENDSWITH, L"0123456789", -1, L"0123", -1, &_nFound, nullptr, nullptr, 0);
            Assert::AreEqual(_nIndex, -1);

            _nIndex = ::FindNLSStringEx(LOCALE_NAME_SYSTEM_DEFAULT, FIND_ENDSWITH, L"0123456789", -1, L"6789", -1, &_nFound, nullptr, nullptr, 0);
            Assert::AreEqual(_nIndex, 6);
            Assert::AreEqual(_nFound, 4);
        }

        TEST_METHOD(FIND_FROMEND验证)
        {
            INT _nFound;
            int _nIndex;
            ...
            _nIndex = ::FindNLSStringEx(LOCALE_NAME_SYSTEM_DEFAULT, FIND_FROMEND, L"6789456789", -1, L"6789", -1, &_nFound, nullptr, nullptr, 0);
            Assert::AreEqual(_nIndex, 6);
            Assert::AreEqual(_nFound, 4);

            _nIndex = ::FindNLSStringEx(LOCALE_NAME_SYSTEM_DEFAULT, FIND_FROMEND, L"0123456789", -1, L"9876", -1, &_nFound, nullptr, nullptr, 0);
            Assert::AreEqual(_nIndex, -1);
        }
    };
}
