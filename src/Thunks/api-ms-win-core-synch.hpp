...
#define YY_SRWLOCK_MASK                                                                            \
    (size_t(                                                                                       \
        YY_SRWLOCK_OWNED | YY_SRWLOCK_CONTENDED | YY_SRWLOCK_SHARED | YY_SRWLOCK_CONTENTION_LOCK))
    ...
    for (;;)
    {
        ...
        else
        {
            auto NewStatus = InterlockedCompareExchange(
                (volatile LONG *)SRWLock, Status & ~YY_SRWLOCK_SHARED, Status);
            if (NewStatus == Status)
                return;
    ...
    for (;;)
    {
        if (Status & YY_SRWLOCK_OWNED)
        {
            ...
            auto CurrentStatus = InterlockedCompareExchange(
                (volatile size_t *)SRWLock, Status & ~YY_SRWLOCK_SHARED, Status);
...
static NTSTATUS __fastcall RtlpWaitOnAddressWithTimeout(
    YY_ADDRESS_WAIT_BLOCK *pWaitBlock,
        ...
        else
        {
            ...
            if (Last == Current)
            {
                ...
                for (;;)
                {
                    const auto Last = InterlockedCompareExchange(
                        ppFirstBlock,
                        (Current & 1) == 0 ? size_t(YY_ADDRESS_GET_BLOCK(Current)) : 0,
                        Current);
...
static void __fastcall YY_RtlpWakeByAddress(
...
#else
{
    ...
    for (;;)
    {
        ...
        if (YY_SRWLOCK_OWNED & SRWLockOld)
        {
            ...
            */
            ...
            if (YY_SRWLOCK_CONTENDED & SRWLockOld)
            {
                ...
                SRWLockNew = (size_t)(&StackWaitBlock) | (SRWLockOld & YY_SRWLOCK_CONTENTION_LOCK) |
                             YY_SRWLOCK_SHARED | YY_SRWLOCK_CONTENDED | YY_SRWLOCK_OWNED;
            ...
            else
            {
                ...
                SRWLockNew =
                    StackWaitBlock.shareCount > 1
                        ? (size_t)(&StackWaitBlock) | YY_SRWLOCK_CONTENTION_LOCK |
                              YY_SRWLOCK_CONTENDED | YY_SRWLOCK_OWNED
        ...
        else
        {
            //尝试获取锁的所有权
            if (InterlockedCompareExchange(
                    (volatile size_t *)SRWLock, SRWLockOld | YY_SRWLOCK_OWNED, SRWLockOld) ==
                SRWLockOld)
            {
                //成功加锁
                return;
...
#else
{
    ...
    {
        if ((OldSRWLock & YY_SRWLOCK_OWNED) &&
            ((OldSRWLock & YY_SRWLOCK_CONTENDED) || YY_SRWLOCK_GET_BLOCK(OldSRWLock) == nullptr))
        {
            // if ( RtlpWaitCouldDeadlock() )
            //    ZwTerminateProcess((HANDLE)0xFFFFFFFF, 0xC000004B);
            ...
            if (OldSRWLock & YY_SRWLOCK_CONTENDED)
            {
                ...
                NewSRWLock = size_t(&StackWaitBlock) | (OldSRWLock & YY_SRWLOCK_CONTENTION_LOCK) |
                             (YY_SRWLOCK_SHARED | YY_SRWLOCK_CONTENDED | YY_SRWLOCK_OWNED);
...
#else
{
    ...
    for (;;)
    {
        ...
        {
            auto NewSRWLock =
                size_t(YY_SRWLOCK_GET_BLOCK(OldSRWLock)) <= 0x10 ? 0 : OldSRWLock - 0x10;
...
EXTERN_C
BOOL WINAPI
EnterSynchronizationBarrier(_Inout_ LPSYNCHRONIZATION_BARRIER lpBarrier, _In_ DWORD dwFlags)
#ifdef YY_Thunks_Defined
    ;
#else
{
