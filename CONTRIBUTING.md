...
| 类型 | 规范 | 示例 |
| --- | --- | --- |
| 命名空间 | 使用 `YY::Thunks` 等分层命名空间 | `YY::Thunks::Fallback` |
| 头文件 | 按 API family / DLL 族命名 | `api-ms-win-core-winrt.hpp` |
| 测试类 | 采用中文业务分组名或按 API/用途分组 | `TEST_CLASS(通用项检测)` |
| 测试方法 | 采用中文场景名，直接表达验证目标 | `TEST_METHOD(符号Thunks检测)` |
| 局部变量 | 常见前缀 `_`，类型/语义结合 | `_uVersion`、`_szResult` |
| 常量 | 常量名使用 `k` 前缀 | `kHex` |
| 宏 | 沿用全大写风格 | `__DEFINE_THUNK` |
